...
for (const width of [390, 1440]) {
  for (const route of routes) {
    test(`link contrast ${width}px ${route}`, async ({ page }) => {
...
for (const width of [390, 1440]) {
  for (const route of ['/', '/en/']) {
    test(`open navigation link contrast ${width}px ${route}`, async ({ page }) => {
      await page.setViewportSize({ width, height: 1000 });
      await page.emulateMedia({ reducedMotion: 'reduce' });
      const response = await page.goto(route);
...
test('focused links have contrasting light and dark indicator bands', async ({ page }, testInfo) => {
  ...
  for (const [name, selector] of [['dark', '.hero-actions a'], ['light', '.footer-main nav a']]) {
    ...
    await expect.poll(indicator).toMatchObject({ visible: true, color: 'rgb(18, 36, 62)', width: '3px', shadow: expect.stringContaining('rgb(255, 255, 255)') });
    expect(await link.evaluate(el => getComputedStyle(el).boxShadow)).toContain('6px');
