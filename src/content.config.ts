...
const isoDate = z.string().regex(/^\d{4}(?:-\d{2}-\d{2})?$/, 'Use YYYY or an ISO date in YYYY-MM-DD format');
...
const publications = defineCollection({
  ...
  schema: ({ image }) => z.object({
    ...
    confirms: z.array(z.string().min(1)).optional(),
    doesNotConfirm: z.array(z.string().min(1)).optional(),
    ...
    if (item.kind === 'award' && (!item.confirms?.length || !item.doesNotConfirm?.length)) {
      ctx.addIssue({ code: 'custom', path: ['confirms'], message: 'Awards require confirms and doesNotConfirm statements' });
    }
  }),
});
