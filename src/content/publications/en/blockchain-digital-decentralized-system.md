...
sources:
  - label: "Original post dated 18 November 2025"
    ...
    type: first-party
  - label: "HealthTAG CMS archive article (26 May 2026 is a migration date, not the original publication date)"
...
For HealthTAG, the confirmed scope is narrower: the Hospital API checks a 15-minute window defined from blockchain state, and blockchain records consent and access events. Details about smart-contract execution should come from documentation for the deployed system version, not from general assumptions about blockchain.
...
> Migration note: the CMS shows 26 May 2026, which matches the earlier migration batch. A HealthTAG post establishes that the original article was published on 18 November 2025, so this entry uses the post date instead of the CMS migration date.
